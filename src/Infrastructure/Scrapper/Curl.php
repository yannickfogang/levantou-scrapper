...
class Curl
{
    ...
    public function Call($url, $countryCode, array $extractRule): bool|string
    {
        ...
        $url = self::API_URL
            ...
            ."&extract_rules=" . urlencode($jsonExtractRule);
    ...
    public function CallJS($url, $countryCode, array $extractRule): bool|string
    {
        $jsonExtractRule = json_encode($extractRule);
        $curl = curl_init();
        $url = self::API_URL
            . "?url=". urlencode($url)
            ."&api_key=". self::API_KEY
            ."&device=desktop&proxy_type=datacenter&render_js=1"
            ."&country=" . $countryCode . '&extract_rules=' . urlencode($jsonExtractRule);
